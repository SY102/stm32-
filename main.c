...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
uint8_t tx_address[5] = {0xE7, 0xE7, 0xE7, 0xE7, 0xE7};    //송신대상 nrf주소
...
void transmit_text(const char* msg)
{
    ...
    if (len > 32) len = 32;     //보낼 메세지 길이 계산후, nrf24최대크기 32바이트로 제한

    if (nrf24_transmit((uint8_t*)msg, len) == 0) {            //데이터 전송 성공
        printf("Sent: %s\r\n", msg);
        HAL_GPIO_WritePin(GPIOC, GPIO_PIN_8, GPIO_PIN_SET);   //LED8 on
        HAL_GPIO_WritePin(GPIOC, GPIO_PIN_6, GPIO_PIN_RESET);
    } else {                                                  //데이터 전송 실패
        printf("Transmit failed: %s\r\n", msg);
        HAL_GPIO_WritePin(GPIOC, GPIO_PIN_6, GPIO_PIN_SET);   //LED6 ON
        HAL_GPIO_WritePin(GPIOC, GPIO_PIN_8, GPIO_PIN_RESET);
    }

    nrf24_flush_tx();                                        //내부 전송 버퍼 비우기
}
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  //nrf24_transmit((uint8_t*)message, strlen(message));
	  transmit_text(message);     //수신처리 함수 호출해 데이터확인,led처리
	  HAL_Delay(1000);
